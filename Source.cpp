...
struct customer {
	...
	void view_available_cars() {
		...
		int i = 0;
		while (!read_data_for_cars.eof()) {
			getline(read_data_for_cars, list_of_cars[i]);
			i++;
			count++;
		}
		count--;
...
struct admin {
	...
	void view_cars() {
		...
		read_data_for_cars.open("Car Data\\list_of_cars.txt");
		int i = 0;
		while (getline(read_data_for_cars, list_of_cars[i])) {
			i++;
			count++;
		}
		read_data_for_cars.close();
	...
	void view_customers() {
		...
		string address;
		string phone_number;
		string list_of_customers[100];
		int count = 0;
		int location = -1;

		ifstream read_data_for_customers;
		read_data_for_customers.open("Customer Data\\Customer List.txt");
		int i = 0;
		while (getline(read_data_for_customers, list_of_customers[i])) {
			i++;
			count++;
		}
	...
	void Remove_Customer() {
		...
		for (int i = CN-1; i < num_of_customers; i++)
		{
	...
	void car_report() {
		...
		retry:
		int choice;
		cout << "Report of which car do you want:";
		cin >> choice;
		if (choice>count)
		{
			goto retry;
